...
class RegistrationController extends AbstractController
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        // On envoi un mail
        $mail->send(
            'no-refly@charlyexpress.com',
            $user->getEmail(),
            'Activation de votre compte chez charlyxpress',
            'register',
            compact('user', 'token')
        );
    ...
    {
        // On verifie si le token est valide, n'a pas expiré et n'a pas été modifier 

        if($jwt->isValid($token) && !$jwt->isExpired($token) && $jwt->check($token, $this->getParameter('app.jwtsecret'))){

            // On récuper le payload 
            $payload = $jwt->getPayload($token);

            // On récupère le user de token 
    ...
    public function resendVerif(JWTService $jwt, SendMailService $mail, UsersRepository $usersRepository): Response
    {
        ...
        if (!$user){
            $this->addFlash('danger', 'Vous devez etre connecté pour accéder cette page');
            return $this->redirectToRoute('app_login');
        }
